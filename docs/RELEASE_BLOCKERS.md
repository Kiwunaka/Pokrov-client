...
summary checks, stack-count consistency, error-free input summary checks, and
explicit source-only no-binary flags plus the next manual maintainer steps
without merging, tagging, pushing, publishing, or uploading anything. The input
summaries must come from the expected ignored `build/` output roots.
