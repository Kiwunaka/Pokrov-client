...
- `powershell -ExecutionPolicy Bypass -File .\scripts\prepare-release-merge-handoff.ps1`
  ...
  handoff report with input SHA-256 fingerprints and explicit source-only
  no-binary flags. The handoff accepts prerequisite summaries only from their
  expected ignored `build/` output roots; it does not merge, tag, push,
