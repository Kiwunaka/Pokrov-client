...
`scripts/render-source-release-notes.ps1` can render the first draft from the
proof manifest generated by `scripts/prepare-source-release.ps1`. Review the
draft, then fill the exact feature status and known limitations before
publishing.
