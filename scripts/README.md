...
- `prepare-release-merge-handoff.ps1`: creates a release merge handoff report
  ...
  read-only input summary checks, and explicit source-only no-binary flags.
  Prerequisite summaries must come from their expected ignored `build/` output
  roots; the helper does not merge, tag, push, publish, or upload anything
