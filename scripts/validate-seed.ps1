...
if (Test-Path -LiteralPath $releaseEvidencePath -PathType Leaf) {
  ...
  foreach ($field in @("source_only_boundary_required", "no_publish_side_effects", "writes_only_ignored_build_output", "ruleset_report_may_be_failing", "failing_ruleset_report_blocks_enforcement_claims", "does_not_replace_full_preflight", "windows_bundle_verifier_required", "requires_input_fingerprints", "requires_preflight_artifact_fingerprints", "requires_preflight_artifact_fingerprint_integrity", "requires_preflight_commit_sha_consistency", "requires_preflight_ref_commit_sha_consistency")) {
  ...
  if (Test-Path -LiteralPath $releaseEvidenceScriptPath -PathType Leaf) {
    ...
    foreach ($requiredPhrase in @("Assert-SourceOnlySummary", "check-github-ruleset.ps1 -ReportOnly -Json", "github_enforcement_claim_allowed", "ships_apk = `$false", "ships_exe = `$false", "store_release = `$false", "trusted_signing_claim = `$false", "official_binary_claim = `$false", "windows_bundle_verifier_ok", "windows_bundle_verifier_summary", "input_fingerprints", "preflight_artifact_fingerprints", "preflight_commit_sha", "preflight_ref_commit_sha", "preflight commit SHA does not match current HEAD", "preflight commit SHA does not match resolved ref commit SHA", "preflight summary is missing artifact fingerprints", "artifact fingerprint mismatch", "SHA256", "ComputeHash", "build\release-evidence")) {
...
if (Test-Path -LiteralPath $publicationDryRunPath -PathType Leaf) {
  ...
  if (Test-Path -LiteralPath $publicationDryRunScriptPath -PathType Leaf) {
    ...
    foreach ($requiredPhrase in @("check-source-release-copy.ps1", "github_enforcement_claim_allowed", "publish_performed = `$false", "tag_push_performed = `$false", "dry_run_only = `$true", "windows_bundle_verifier_ok", "windows_bundle_verifier_summary", "input_fingerprints", "evidence_bundle_input_fingerprints", "evidence_bundle_preflight_artifact_fingerprints", "evidence_bundle_preflight_commit_sha", "evidence_bundle_preflight_ref_commit_sha", "evidence commit SHA does not match preflight commit SHA", "evidence preflight commit SHA does not match resolved ref commit SHA", "evidence bundle is missing input fingerprints", "evidence bundle is missing preflight artifact fingerprints", "artifact fingerprint mismatch", "SHA256", "ComputeHash", "build\source-release-publication")) {
...
if (Test-Path -LiteralPath $sourceTagReadinessPath -PathType Leaf) {
  ...
  if (Test-Path -LiteralPath $sourceTagReadinessScriptPath -PathType Leaf) {
    ...
    foreach ($requiredPhrase in @("release-blocker-inventory.seed.json", "source-release-readiness.seed.json", "latest_candidate", "latest_stacked_pr", "read_only = `$true", "input_fingerprints", "ComputeHash", "requested tag does not match latest blocker inventory candidate", "source readiness milestone evidence does not match latest stacked PR", "source readiness milestone is missing evidence", "source readiness milestone is missing status", "source readiness milestone is missing scope", "release blocker inventory has unsafe source-only release flags", "source readiness milestone has unsafe source-only release flags", "open blocker is missing id", "blocker `$blockerId is missing required_before_tag=true", "open blocker `$blockerId is missing status", "open blocker `$blockerId is missing evidence", "error_count", "errors", "open_blocker_count", "required_before_tag", "evidence", "build\source-tag-readiness", "exit 2")) {
...
if (Test-Path -LiteralPath $releaseStackGithubStatusPath -PathType Leaf) {
  ...
  foreach ($field in @("read_only", "no_merge", "no_git_push", "no_github_api_mutation", "uses_pr_status_snapshot", "writes_only_ignored_build_output", "requires_clean_prs", "requires_ci_success", "requires_pull_request_urls", "requires_expected_repository_pr_urls", "requires_per_pr_status_check_evidence")) {
  ...
  if (Test-Path -LiteralPath $releaseStackGithubStatusScriptPath -PathType Leaf) {
    ...
    foreach ($requiredPhrase in @("release-stack-github-status.seed.json", "release-merge-order.seed.json", "gh pr list", "url", "latest_pr_url", "expected_pr_url_prefix", "pull request URL is missing", "pull request URL does not match expected repository", "successful_check_count", "failed_check_count", "required_status_check_count", "checks = @(`$prChecks)", "mergeStateStatus", "statusCheckRollup", "build\release-stack-github-status", "github_status_ok = `$true")) {
