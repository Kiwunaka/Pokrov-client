...
if (Test-Path -LiteralPath $releaseMergeHandoffPath -PathType Leaf) {
  ...
  if (Test-Path -LiteralPath $releaseMergeHandoffScriptPath -PathType Leaf) {
    ...
    foreach ($requiredPhrase in @("release-merge-handoff.seed.json", "merge_order_ok", "github_status_ok", "publication_dry_run_ok", "ready_for_tag", "ready_for_manual_review", "windows_bundle_verifier_ok", "input_fingerprints", "source_only = `$true", "no_apk = `$true", "no_exe = `$true", "no_store_release = `$true", "no_trusted_signing_claim = `$true", "Assert-BuildInputPath", "input_roots", "SHA256", "ComputeHash", "handoff_ready_for_maintainer", "build\release-merge-handoff", "manual_merge_required = `$true")) {
