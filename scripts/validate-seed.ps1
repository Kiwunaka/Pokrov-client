...
if (Test-Path -LiteralPath $sourceTagReadinessPath -PathType Leaf) {
  ...
  if (Test-Path -LiteralPath $sourceTagReadinessScriptPath -PathType Leaf) {
    ...
    foreach ($requiredPhrase in @("release-blocker-inventory.seed.json", "source-release-readiness.seed.json", "latest_candidate", "latest_stacked_pr", "requested tag does not match latest blocker inventory candidate", "source readiness milestone evidence does not match latest stacked PR", "release blocker inventory has unsafe source-only release flags", "source readiness milestone has unsafe source-only release flags", "open blocker is missing id", "open blocker `$blockerId is missing status", "open blocker `$blockerId is missing evidence", "error_count", "errors", "open_blocker_count", "required_before_tag", "evidence", "build\source-tag-readiness", "exit 2")) {
