...
if (Test-Path -LiteralPath $githubRulesetPath -PathType Leaf) {
  ...
  if (Test-Path -LiteralPath $githubRulesetScriptPath -PathType Leaf) {
    ...
    foreach ($requiredPhrase in @("gh api", "-ReportOnly", "read_only", "required_status_checks", "covered_required_status_checks", "coveredRequiredChecks", "branch_protection", "ruleset")) {
