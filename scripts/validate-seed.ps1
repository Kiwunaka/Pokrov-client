...
if (Test-Path -LiteralPath $releaseStackGithubStatusPath -PathType Leaf) {
  ...
  foreach ($field in @("read_only", "no_merge", "no_git_push", "no_github_api_mutation", "uses_pr_status_snapshot", "writes_only_ignored_build_output", "requires_clean_prs", "requires_ci_success", "requires_pull_request_urls")) {
  ...
  if (Test-Path -LiteralPath $releaseStackGithubStatusScriptPath -PathType Leaf) {
    ...
    foreach ($requiredPhrase in @("release-stack-github-status.seed.json", "release-merge-order.seed.json", "gh pr list", "url", "latest_pr_url", "pull request URL is missing", "mergeStateStatus", "statusCheckRollup", "build\release-stack-github-status", "github_status_ok = `$true")) {
