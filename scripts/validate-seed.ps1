...
if (Test-Path -LiteralPath $releaseEvidencePath -PathType Leaf) {
  ...
  if (Test-Path -LiteralPath $releaseEvidenceScriptPath -PathType Leaf) {
    ...
    foreach ($requiredPhrase in @("Assert-SourceOnlySummary", "check-github-ruleset.ps1 -ReportOnly -Json", "github_enforcement_claim_allowed", "github_ruleset_report", "ships_apk = `$false", "ships_exe = `$false", "store_release = `$false", "trusted_signing_claim = `$false", "official_binary_claim = `$false", "windows_bundle_verifier_ok", "windows_bundle_verifier_summary", "input_fingerprints", "Assert-RulesetReportShape", "ruleset report without schema_version 1", "ruleset report that is not read-only", "ruleset report without ok status", "preflight_artifact_fingerprints", "preflight_commit_sha", "preflight_ref_commit_sha", "preflight commit SHA does not match current HEAD", "preflight commit SHA does not match resolved ref commit SHA", "preflight summary is missing artifact fingerprints", "artifact fingerprint mismatch", "SHA256", "ComputeHash", "build\release-evidence")) {
