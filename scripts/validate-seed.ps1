...
if (Test-Path -LiteralPath $releaseEvidencePath -PathType Leaf) {
  ...
  foreach ($field in @("source_only_boundary_required", "no_publish_side_effects", "writes_only_ignored_build_output", "ruleset_report_may_be_failing", "failing_ruleset_report_blocks_enforcement_claims", "does_not_replace_full_preflight", "windows_bundle_verifier_required", "requires_input_fingerprints", "requires_ruleset_report_input_fingerprint_when_present", "requires_ruleset_report_shape", "requires_ruleset_report_target", "requires_ruleset_report_ok_consistency", "requires_ruleset_report_check_entry_shape", "requires_ruleset_report_required_status_checks", "requires_ruleset_report_required_status_check_coverage", "requires_preflight_artifact_fingerprints", "requires_preflight_artifact_fingerprint_integrity", "requires_preflight_commit_sha_consistency", "requires_preflight_ref_commit_sha_consistency")) {
  ...
  if (Test-Path -LiteralPath $releaseEvidenceScriptPath -PathType Leaf) {
    ...
    foreach ($requiredPhrase in @("Assert-SourceOnlySummary", "check-github-ruleset.ps1 -ReportOnly -Json", "github_enforcement_claim_allowed", "github_ruleset_report", "ships_apk = `$false", "ships_exe = `$false", "store_release = `$false", "trusted_signing_claim = `$false", "official_binary_claim = `$false", "windows_bundle_verifier_ok", "windows_bundle_verifier_summary", "input_fingerprints", "Assert-RulesetReportShape", "ruleset report without schema_version 1", "ruleset report that is not read-only", "ruleset report without ok status", "ruleset report repository mismatch", "ruleset report branch mismatch", "ruleset report ok status without checks", "ruleset report ok status with failed checks", "ruleset report check entry shape mismatch", "ruleset report required status checks mismatch", "ruleset report required status check coverage mismatch", "preflight_artifact_fingerprints", "preflight_commit_sha", "preflight_ref_commit_sha", "preflight commit SHA does not match current HEAD", "preflight commit SHA does not match resolved ref commit SHA", "preflight summary is missing artifact fingerprints", "artifact fingerprint mismatch", "SHA256", "ComputeHash", "build\release-evidence")) {
...
if (Test-Path -LiteralPath $publicationDryRunPath -PathType Leaf) {
  ...
  foreach ($field in @("read_only", "dry_run_only", "no_github_release_publish", "no_tag_push", "writes_only_ignored_build_output", "requires_source_only_evidence_bundle", "requires_release_copy_check", "requires_input_fingerprints", "requires_evidence_bundle_input_fingerprints", "requires_evidence_bundle_preflight_input_fingerprint_integrity", "requires_evidence_bundle_ruleset_report_input_fingerprint_integrity", "requires_evidence_bundle_ruleset_report_shape", "requires_evidence_bundle_ruleset_report_target", "requires_evidence_bundle_ruleset_report_ok_consistency", "requires_evidence_bundle_ruleset_report_check_entry_shape", "requires_evidence_bundle_ruleset_report_required_status_checks", "requires_evidence_bundle_ruleset_report_required_status_check_coverage", "requires_evidence_bundle_preflight_artifact_fingerprints", "github_enforcement_claim_requires_bundle_approval", "windows_bundle_verifier_claim_requires_bundle_proof", "requires_evidence_bundle_preflight_artifact_fingerprint_integrity", "requires_evidence_bundle_preflight_commit_sha_consistency", "requires_evidence_bundle_preflight_ref_commit_sha_consistency")) {
