...
def test_source_tag_readiness_reports_ready_candidate(tmp_path: Path) -> None:
    ...
    assert "Source tag readiness: READY" in result.stdout

    summary = json.loads(
        (out_dir / "v0.172.0-source-tag-readiness.json").read_text(
            encoding="utf-8-sig"
        )
    )
    assert summary["tag"] == "v0.172.0-source"
    assert summary["read_only"] is True
    assert summary["ready_for_tag"] is True
    ...
    assert summary["tag_creation_allowed"] is True
...
def test_source_tag_readiness_blocks_milestone_evidence_pr_mismatch(
    ...
    try:
        ...
        for milestone in readiness["milestones"]:
            if milestone["tag"] == "v0.172.0-source":
    ...
    assert summary["latest_stacked_pr"] == 193
...
def test_source_tag_readiness_blocks_milestone_evidence_repo_mismatch(
    ...
    try:
        ...
        for milestone in readiness["milestones"]:
            if milestone["tag"] == "v0.172.0-source":
                milestone["evidence"] = "https://github.com/example/fork/pull/193"
                break
        _write_json(readiness_path, readiness)
    ...
    assert (
        ...
        == "https://github.com/Kiwunaka/Pokrov-client/pull/193"
    )
    assert summary["milestone_evidence"].endswith("/pull/193")
    assert (
        "source readiness milestone evidence does not match expected repository PR URL"
        in summary["errors"]
    )
...
def test_source_tag_readiness_blocks_open_blockers_without_evidence(
    ...
    try:
        inventory = json.loads(inventory_path.read_text(encoding="utf-8"))
        inventory["blockers"][0]["status"] = "pending_maintainer_review"
    ...
    assert result.returncode == 2
    assert "blocker merge_stacked_pr_sequence is missing evidence" in (
        result.stdout + result.stderr
    )
    assert "blocker merge_stacked_pr_sequence is missing evidence" in summary[
        "errors"
    ]
...
def test_source_tag_readiness_blocks_tag_creation_allowed_with_unready_status(
    ...
    try:
        inventory = json.loads(inventory_path.read_text(encoding="utf-8"))
        inventory["status"] = "not_ready_for_tag"
        inventory["tracked_candidates"]["tag_creation_allowed"] = True
...
def test_source_tag_readiness_blocks_tag_creation_allowed_with_pending_milestone(
    ...
    try:
        ...
        for milestone in readiness["milestones"]:
            if milestone["tag"] == "v0.172.0-source":
                milestone["status"] = "stacked_pr_pending_not_tagged"
                break
        _write_json(readiness_path, readiness)
...
def test_source_tag_readiness_blocks_open_blockers_without_id(
    ...
    assert result.returncode == 2
    assert "blocker is missing id" in result.stdout + result.stderr
    assert "blocker is missing id" in summary["errors"]
