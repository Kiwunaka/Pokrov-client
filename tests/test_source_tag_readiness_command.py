...
def test_source_tag_readiness_reports_current_blockers(tmp_path: Path) -> None:
    ...
    summary = json.loads(
        (out_dir / "v0.147.0-source-tag-readiness.json").read_text(
            encoding="utf-8-sig"
        )
    )
    assert summary["tag"] == "v0.147.0-source"
...
def test_source_tag_readiness_blocks_milestone_evidence_pr_mismatch(
    ...
    try:
        ...
        for milestone in readiness["milestones"]:
            if milestone["tag"] == "v0.147.0-source":
    ...
    assert summary["latest_stacked_pr"] == 168
...
def test_source_tag_readiness_blocks_milestone_evidence_repo_mismatch(
    ...
    try:
        ...
        for milestone in readiness["milestones"]:
            if milestone["tag"] == "v0.147.0-source":
                milestone["evidence"] = "https://github.com/example/fork/pull/168"
                break
        _write_json(readiness_path, readiness)
    ...
    assert (
        ...
        == "https://github.com/Kiwunaka/Pokrov-client/pull/168"
    )
    assert summary["milestone_evidence"].endswith("/pull/168")
    assert (
        "source readiness milestone evidence does not match expected repository PR URL"
        in summary["errors"]
    )
