...
def _release_handoff_summary(
    ...
    return {
        ...
        "latest_candidate": "v0.168.0-source",
        "latest_pr": 189,
        "latest_pr_url": "https://github.com/Kiwunaka/Pokrov-client/pull/189",
        "publication_dry_run_ok": True,
        "publication_dry_run_commit_sha": publication_summary["commit_sha"],
        "publication_dry_run_source_archive": publication_summary["source_archive"],
        "publication_dry_run_source_archive_sha256": publication_summary[
            "source_archive_sha256"
        ],
        "publication_dry_run_evidence_bundle_preflight_commit_sha": (
            publication_summary["evidence_bundle_preflight_commit_sha"]
        ),
        "publication_dry_run_evidence_bundle_preflight_ref_commit_sha": (
            publication_summary["evidence_bundle_preflight_ref_commit_sha"]
        ),
        "windows_bundle_verifier_ok": publication_summary["windows_bundle_verifier_ok"],
        "windows_bundle_verifier_summary": publication_summary[
            "windows_bundle_verifier_summary"
        ],
        "github_ruleset_report": publication_summary["github_ruleset_report"],
...
def _publication_dry_run_summary(tmp_path: Path, ready: bool = True) -> dict:
    ...
    ruleset_report = evidence_root / "github-ruleset-report.json"
    ...
    _write_json(
        ruleset_report,
        {
            ...
            "branch": "main",
            "checked_at": _fresh_generated_at(),
            ...
            "checks": [
                {"name": "ruleset:active", "status": "pass"},
    ...
    return {
        ...
        "github_ruleset_report": str(ruleset_report),
        "source_archive": "source.zip",
        "source_archive_sha256": _sha256(source_archive),
        ...
        "evidence_bundle_input_fingerprints": {
            ...
            "github_ruleset_report": {
                "path": str(ruleset_report),
                "sha256": _sha256(ruleset_report),
            },
        },
        "evidence_bundle_preflight_artifact_fingerprints": {
            "proof_manifest": {
...
def _write_input_summaries(
    ...
    *,
    ...
    publication_path = (
        ...
        / "test-inputs"
        / suffix
        / "v0.168.0-source-publication-dry-run.json"
    )
...
def test_source_publication_packet_seed_defines_manual_publish_policy() -> None:
    ...
    assert (
        seed["policy"][
            "carries_release_handoff_publication_dry_run_source_archive_name"
        ]
        is True
    )
    assert (
        seed["policy"][
            "requires_release_handoff_publication_dry_run_source_archive_sha_binding"
        ]
        is True
    )
    assert (
        seed["policy"]["carries_release_handoff_publication_dry_run_source_archive_sha"]
        is True
    )
    assert (
        seed["policy"][
            "requires_release_handoff_publication_dry_run_commit_sha_binding"
        ]
        is True
    )
    assert (
        seed["policy"]["carries_release_handoff_publication_dry_run_commit_sha"]
        is True
    )
    assert (
        seed["policy"][
    ...
    assert (
        seed["policy"][
            "carries_release_handoff_publication_dry_run_evidence_bundle_preflight_commit_sha"
        ]
        is True
    )
    assert (
        seed["policy"][
            "requires_release_handoff_publication_dry_run_evidence_bundle_preflight_ref_commit_sha_binding"
        ]
        is True
    )
    assert (
        seed["policy"][
            "carries_release_handoff_publication_dry_run_evidence_bundle_preflight_ref_commit_sha"
        ]
        is True
    )
    assert (
        seed["policy"]["requires_release_handoff_windows_bundle_verifier_binding"]
        is True
    )
    assert (
        seed["policy"]["carries_release_handoff_windows_bundle_verifier_proof"]
        is True
    )
    assert (
        seed["policy"]["requires_release_handoff_github_ruleset_report_binding"]
        is True
    )
    assert (
        seed["policy"]["carries_release_handoff_github_ruleset_report_proof"]
        is True
    )
    assert (
        seed["policy"][
...
def test_source_publication_packet_script_is_read_only() -> None:
    ...
    for phrase in (
        ...
        "release_handoff_windows_bundle_verifier_ok",
        "release_handoff_windows_bundle_verifier_summary",
        "release_handoff_github_ruleset_report",
        "release handoff summary is missing publication_dry_run_source_archive_sha256",
        "release handoff publication dry-run source archive SHA mismatch",
        "release handoff summary is missing publication_dry_run_evidence_bundle_preflight_commit_sha",
        "release handoff publication dry-run evidence bundle preflight commit SHA mismatch",
        "release handoff summary is missing publication_dry_run_evidence_bundle_preflight_ref_commit_sha",
        "release handoff publication dry-run evidence bundle preflight ref commit SHA mismatch",
        "release handoff summary is missing windows_bundle_verifier_ok",
        "release handoff Windows bundle verifier status mismatch",
        "release handoff summary is missing windows_bundle_verifier_summary",
        "release handoff Windows bundle verifier summary mismatch",
        "publication dry-run summary is missing github_ruleset_report",
        "release handoff summary is missing github_ruleset_report",
        "release handoff GitHub ruleset report mismatch",
        "release handoff publication dry-run source archive name mismatch",
        "release handoff publication dry-run commit SHA mismatch",
...
def test_source_publication_packet_command_writes_review_packet(
    ...
    try:
        ...
        packet = json.loads(
            (
                out_dir / "v0.168.0-source" / "source-publication-packet.json"
    ...
    assert packet["latest_candidate"] == "v0.168.0-source"
    assert packet["latest_pr"] == 189
    assert packet["latest_pr_url"] == "https://github.com/Kiwunaka/Pokrov-client/pull/189"
    ...
    assert (
        packet["release_handoff_publication_dry_run_source_archive"]
        == handoff["publication_dry_run_source_archive"]
    )
    assert (
        packet["release_handoff_publication_dry_run_commit_sha"]
        == handoff["publication_dry_run_commit_sha"]
    )
    assert (
        packet["release_handoff_publication_dry_run_source_archive_sha256"]
        == handoff["publication_dry_run_source_archive_sha256"]
    )
    assert (
        packet[
            "release_handoff_publication_dry_run_evidence_bundle_preflight_commit_sha"
        ]
        == handoff["publication_dry_run_evidence_bundle_preflight_commit_sha"]
    )
    assert (
        packet[
            "release_handoff_publication_dry_run_evidence_bundle_preflight_ref_commit_sha"
        ]
        == handoff["publication_dry_run_evidence_bundle_preflight_ref_commit_sha"]
    )
    assert (
        packet["release_handoff_windows_bundle_verifier_ok"]
        == handoff["windows_bundle_verifier_ok"]
    )
    assert (
        packet["release_handoff_windows_bundle_verifier_summary"]
        == handoff["windows_bundle_verifier_summary"]
    )
    assert packet["release_handoff_github_ruleset_report"] == handoff[
        "github_ruleset_report"
    ]
...
def test_source_publication_packet_rejects_handoff_publication_source_archive_name_mismatch(
    tmp_path: Path,
) -> None:
    handoff_path, publication_path = _write_input_summaries(tmp_path)
    handoff = json.loads(handoff_path.read_text(encoding="utf-8"))
    handoff["publication_dry_run_source_archive"] = "different-source.zip"
    _write_json(handoff_path, handoff)
    out_dir = (
        ROOT
        / "build"
        / "source-publication-packet"
        / "test-handoff-source-archive-name-mismatch"
    )
    shutil.rmtree(out_dir, ignore_errors=True)
...
def test_source_publication_packet_rejects_missing_handoff_publication_commit_sha(
    tmp_path: Path,
) -> None:
    handoff_path, publication_path = _write_input_summaries(tmp_path)
    handoff = json.loads(handoff_path.read_text(encoding="utf-8"))
    handoff.pop("publication_dry_run_commit_sha")
    _write_json(handoff_path, handoff)
    out_dir = (
        ROOT
        / "build"
        / "source-publication-packet"
        / "test-handoff-publication-commit-sha-missing"
    )
    shutil.rmtree(out_dir, ignore_errors=True)
    ...
    assert result.returncode == 2
    assert "release handoff summary is missing publication_dry_run_commit_sha" in packet[
        "blocking_errors"
    ]


def test_source_publication_packet_rejects_handoff_publication_commit_sha_mismatch(
    tmp_path: Path,
) -> None:
    handoff_path, publication_path = _write_input_summaries(tmp_path)
    handoff = json.loads(handoff_path.read_text(encoding="utf-8"))
    handoff["publication_dry_run_commit_sha"] = "b" * 40
    _write_json(handoff_path, handoff)
    out_dir = (
        ROOT
        / "build"
        / "source-publication-packet"
        / "test-handoff-publication-commit-sha-mismatch"
    )
    shutil.rmtree(out_dir, ignore_errors=True)
    ...
    assert result.returncode == 2
    assert "release handoff publication dry-run commit SHA mismatch" in packet[
        "blocking_errors"
    ]


def test_source_publication_packet_rejects_missing_handoff_publication_source_archive_sha(
    tmp_path: Path,
) -> None:
    handoff_path, publication_path = _write_input_summaries(tmp_path)
    handoff = json.loads(handoff_path.read_text(encoding="utf-8"))
    handoff.pop("publication_dry_run_source_archive_sha256")
    _write_json(handoff_path, handoff)
    out_dir = (
        ROOT
        / "build"
        / "source-publication-packet"
        / "test-handoff-publication-source-archive-sha-missing"
    )
    shutil.rmtree(out_dir, ignore_errors=True)
...
def test_source_publication_packet_rejects_handoff_publication_source_archive_sha_mismatch(
    tmp_path: Path,
) -> None:
    handoff_path, publication_path = _write_input_summaries(tmp_path)
    handoff = json.loads(handoff_path.read_text(encoding="utf-8"))
    handoff["publication_dry_run_source_archive_sha256"] = "b" * 64
    _write_json(handoff_path, handoff)
    out_dir = (
        ROOT
        / "build"
        / "source-publication-packet"
        / "test-handoff-publication-source-archive-sha-mismatch"
    )
    shutil.rmtree(out_dir, ignore_errors=True)
    ...
    assert result.returncode == 2
    assert "release handoff publication dry-run source archive SHA mismatch" in packet[
        "blocking_errors"
    ]


def test_source_publication_packet_rejects_missing_handoff_publication_preflight_commit_sha(
    tmp_path: Path,
) -> None:
    handoff_path, publication_path = _write_input_summaries(tmp_path)
    handoff = json.loads(handoff_path.read_text(encoding="utf-8"))
    handoff.pop("publication_dry_run_evidence_bundle_preflight_commit_sha")
    _write_json(handoff_path, handoff)
    out_dir = (
        ROOT
        / "build"
        / "source-publication-packet"
        / "test-handoff-publication-preflight-commit-sha-missing"
    )
    shutil.rmtree(out_dir, ignore_errors=True)
...
def test_source_publication_packet_rejects_handoff_publication_preflight_commit_sha_mismatch(
    tmp_path: Path,
) -> None:
    handoff_path, publication_path = _write_input_summaries(tmp_path)
    handoff = json.loads(handoff_path.read_text(encoding="utf-8"))
    handoff["publication_dry_run_evidence_bundle_preflight_commit_sha"] = "b" * 40
    _write_json(handoff_path, handoff)
    out_dir = (
        ROOT
        / "build"
        / "source-publication-packet"
        / "test-handoff-publication-preflight-commit-sha-mismatch"
    )
    shutil.rmtree(out_dir, ignore_errors=True)
...
def test_source_publication_packet_rejects_missing_handoff_publication_preflight_ref_commit_sha(
    tmp_path: Path,
) -> None:
    handoff_path, publication_path = _write_input_summaries(tmp_path)
    handoff = json.loads(handoff_path.read_text(encoding="utf-8"))
    handoff.pop("publication_dry_run_evidence_bundle_preflight_ref_commit_sha")
    _write_json(handoff_path, handoff)
    out_dir = (
        ROOT
        / "build"
        / "source-publication-packet"
        / "test-handoff-publication-preflight-ref-commit-sha-missing"
    )
    shutil.rmtree(out_dir, ignore_errors=True)
...
def test_source_publication_packet_rejects_handoff_publication_preflight_ref_commit_sha_mismatch(
    tmp_path: Path,
) -> None:
    handoff_path, publication_path = _write_input_summaries(tmp_path)
    handoff = json.loads(handoff_path.read_text(encoding="utf-8"))
    handoff["publication_dry_run_evidence_bundle_preflight_ref_commit_sha"] = "b" * 40
    _write_json(handoff_path, handoff)
    out_dir = (
        ROOT
        / "build"
        / "source-publication-packet"
        / "test-handoff-publication-preflight-ref-commit-sha-mismatch"
    )
    shutil.rmtree(out_dir, ignore_errors=True)
...
def test_source_publication_packet_rejects_missing_handoff_windows_bundle_verifier_ok(
    tmp_path: Path,
) -> None:
    handoff_path, publication_path = _write_input_summaries(tmp_path)
    handoff = json.loads(handoff_path.read_text(encoding="utf-8"))
    handoff.pop("windows_bundle_verifier_ok")
    _write_json(handoff_path, handoff)
    out_dir = (
        ROOT
        / "build"
        / "source-publication-packet"
        / "test-handoff-windows-bundle-verifier-ok-missing"
    )
    shutil.rmtree(out_dir, ignore_errors=True)
    ...
    assert result.returncode == 2
    assert "release handoff summary is missing windows_bundle_verifier_ok" in packet[
        "blocking_errors"
    ]


def test_source_publication_packet_rejects_handoff_windows_bundle_verifier_ok_mismatch(
    tmp_path: Path,
) -> None:
    handoff_path, publication_path = _write_input_summaries(tmp_path)
    handoff = json.loads(handoff_path.read_text(encoding="utf-8"))
    handoff["windows_bundle_verifier_ok"] = False
    _write_json(handoff_path, handoff)
    out_dir = (
        ROOT
        / "build"
        / "source-publication-packet"
        / "test-handoff-windows-bundle-verifier-ok-mismatch"
    )
    shutil.rmtree(out_dir, ignore_errors=True)
    ...
    assert result.returncode == 2
    assert "release handoff Windows bundle verifier status mismatch" in packet[
        "blocking_errors"
    ]


def test_source_publication_packet_rejects_missing_handoff_windows_bundle_verifier_summary(
    tmp_path: Path,
) -> None:
    handoff_path, publication_path = _write_input_summaries(tmp_path)
    handoff = json.loads(handoff_path.read_text(encoding="utf-8"))
    handoff.pop("windows_bundle_verifier_summary")
    _write_json(handoff_path, handoff)
    out_dir = (
        ROOT
        / "build"
        / "source-publication-packet"
        / "test-handoff-windows-bundle-verifier-summary-missing"
    )
    shutil.rmtree(out_dir, ignore_errors=True)
    ...
    assert result.returncode == 2
    assert "release handoff summary is missing windows_bundle_verifier_summary" in packet[
        "blocking_errors"
    ]


def test_source_publication_packet_rejects_handoff_windows_bundle_verifier_summary_mismatch(
    tmp_path: Path,
) -> None:
    handoff_path, publication_path = _write_input_summaries(tmp_path)
    handoff = json.loads(handoff_path.read_text(encoding="utf-8"))
    handoff["windows_bundle_verifier_summary"] = "build/windows-bundle-verifier/other.json"
    _write_json(handoff_path, handoff)
    out_dir = (
        ROOT
        / "build"
        / "source-publication-packet"
        / "test-handoff-windows-bundle-verifier-summary-mismatch"
    )
    shutil.rmtree(out_dir, ignore_errors=True)
    ...
    assert result.returncode == 2
    assert "release handoff Windows bundle verifier summary mismatch" in packet[
        "blocking_errors"
    ]


def test_source_publication_packet_rejects_missing_handoff_github_ruleset_report(
    tmp_path: Path,
) -> None:
    handoff_path, publication_path = _write_input_summaries(tmp_path)
    handoff = json.loads(handoff_path.read_text(encoding="utf-8"))
    handoff.pop("github_ruleset_report")
    _write_json(handoff_path, handoff)
    out_dir = (
        ROOT
        / "build"
        / "source-publication-packet"
        / "test-handoff-github-ruleset-report-missing"
    )
    shutil.rmtree(out_dir, ignore_errors=True)
    ...
    assert result.returncode == 2
    assert "release handoff summary is missing github_ruleset_report" in packet[
        "blocking_errors"
    ]


def test_source_publication_packet_rejects_handoff_github_ruleset_report_mismatch(
    tmp_path: Path,
) -> None:
    handoff_path, publication_path = _write_input_summaries(tmp_path)
    handoff = json.loads(handoff_path.read_text(encoding="utf-8"))
    handoff["github_ruleset_report"] = "build/release-evidence/other-ruleset.json"
    _write_json(handoff_path, handoff)
    out_dir = (
        ROOT
        / "build"
        / "source-publication-packet"
        / "test-handoff-github-ruleset-report-mismatch"
    )
    shutil.rmtree(out_dir, ignore_errors=True)
    ...
    assert result.returncode == 2
    assert "release handoff GitHub ruleset report mismatch" in packet[
        "blocking_errors"
    ]


def test_source_publication_packet_rejects_missing_publication_github_ruleset_report(
    tmp_path: Path,
) -> None:
    handoff_path, publication_path = _write_input_summaries(tmp_path)
    publication = json.loads(publication_path.read_text(encoding="utf-8"))
    publication.pop("github_ruleset_report")
    publication["evidence_bundle_input_fingerprints"].pop("github_ruleset_report")
    _write_json(publication_path, publication)
    out_dir = (
        ROOT
        / "build"
        / "source-publication-packet"
        / "test-publication-github-ruleset-report-missing"
    )
    shutil.rmtree(out_dir, ignore_errors=True)
    ...
    assert result.returncode == 2
    assert "publication dry-run summary is missing github_ruleset_report" in packet[
        "blocking_errors"
    ]
...
def test_source_publication_packet_rejects_release_notes_without_claim_guards(
    ...
    release_notes_path.write_text("# v0.168.0-source\n", encoding="utf-8")
...
def test_source_publication_packet_rejects_non_build_output(tmp_path: Path) -> None:
    ...
    assert not (out_dir / "v0.168.0-source" / "source-publication-packet.json").exists()
