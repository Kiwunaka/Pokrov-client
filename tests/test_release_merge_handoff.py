...
def _merge_order_summary(ok: bool = True) -> dict:
    return {
        ...
        "stack_count": 22,
        "latest_pr": 82,
        "latest_candidate": "v0.62.0-source",
        "errors": [] if ok else ["PR #82 base must equal previous head"],
    }
...
def _github_status_summary(ok: bool = True) -> dict:
    return {
        ...
        "latest_pr": 82,
        "latest_candidate": "v0.62.0-source",
        "clean_pr_count": 16 if ok else 15,
        "draft_pr_count": 0,
        "unclean_pr_count": 0 if ok else 1,
        "successful_check_count": 66 if ok else 65,
        "failed_check_count": 0 if ok else 1,
        "errors": [] if ok else ["PR #82 check 'Flutter analyze and tests' is FAILURE"],
    }
    ...
    *,
    ...
    if canonical_roots:
        ...
        tag_path = (
            ...
            / "test-inputs"
            / suffix
            / "v0.62.0-source-tag-readiness.json"
        )
        publication_path = (
            ROOT
            / "build"
            ...
            / "test-inputs"
            / suffix
            / "v0.62.0-source-publication-dry-run.json"
        )
    else:
        merge_path = tmp_path / "release-merge-order.json"
        github_path = tmp_path / "release-stack-github-status.json"
        tag_path = tmp_path / "v0.62.0-source-tag-readiness.json"
        publication_path = tmp_path / "v0.62.0-source-publication-dry-run.json"
...
def test_release_merge_handoff_writes_handoff_summary(tmp_path: Path) -> None:
    ...
    assert summary["latest_candidate"] == "v0.62.0-source"
    assert summary["latest_pr"] == 82
