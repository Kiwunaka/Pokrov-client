...
def _merge_order_summary(ok: bool = True) -> dict:
    return {
        ...
        "stack_count": 54,
        "latest_pr": 115,
        "latest_candidate": "v0.94.0-source",
        "errors": [] if ok else ["PR #115 base must equal previous head"],
    }
...
def _github_status_summary(ok: bool = True) -> dict:
    return {
        ...
        "stack_count": 54,
        "latest_pr": 115,
        "latest_pr_url": "https://github.com/Kiwunaka/Pokrov-client/pull/115",
        "expected_pr_url_prefix": "https://github.com/Kiwunaka/Pokrov-client/pull/",
        "latest_candidate": "v0.94.0-source",
        "clean_pr_count": 47 if ok else 46,
        "draft_pr_count": 0,
        "unclean_pr_count": 0 if ok else 1,
        "successful_check_count": 162 if ok else 161,
        "failed_check_count": 0 if ok else 1,
        "errors": [] if ok else ["PR #115 check 'Flutter analyze and tests' is FAILURE"],
    }
...
def _publication_dry_run_summary(ok: bool = True) -> dict:
    return {
        ...
        "input_fingerprints": {
            "evidence_bundle": {
                "path": "build/release-evidence/v0.94.0-source/release-evidence.json",
                "sha256": "c" * 64,
            },
            "release_notes": {
                "path": "build/source-release/v0.94.0-source/release-notes.md",
                "sha256": "d" * 64,
            },
        },
        "evidence_bundle_input_fingerprints": {
            "preflight_summary": {
                "path": "build/source-release-preflight/v0.94.0-source/preflight.json",
                "sha256": "e" * 64,
            },
        },
    ...
    *,
    ...
    if canonical_roots:
        ...
        tag_path = (
            ...
            / "test-inputs"
            / suffix
            / "v0.94.0-source-tag-readiness.json"
        )
        publication_path = (
            ROOT
            / "build"
            ...
            / "test-inputs"
            / suffix
            / "v0.94.0-source-publication-dry-run.json"
        )
    else:
        merge_path = tmp_path / "release-merge-order.json"
        github_path = tmp_path / "release-stack-github-status.json"
        tag_path = tmp_path / "v0.94.0-source-tag-readiness.json"
        publication_path = tmp_path / "v0.94.0-source-publication-dry-run.json"
...
def test_release_merge_handoff_writes_handoff_summary(tmp_path: Path) -> None:
    ...
    assert summary["latest_candidate"] == "v0.94.0-source"
    ...
    assert summary["blocker_inventory_latest_candidate"] == "v0.94.0-source"
    assert summary["blocker_inventory_latest_pr"] == 115
...
def test_release_merge_handoff_blocks_input_summary_errors(
    ...
    merge_summary["errors"] = ["PR #115 base was stale when checked"]
    ...
    assert summary["input_errors"] == ["PR #115 base was stale when checked"]
...
def test_release_merge_handoff_blocks_latest_pr_url_wrong_repository(
    ...
    github_summary = json.loads(github_path.read_text(encoding="utf-8"))
    github_summary["latest_pr_url"] = "https://github.com/example/fork/pull/115"
    _write_json(github_path, github_summary)
    out_dir = ROOT / "build" / "release-merge-handoff" / "test-output"
    shutil.rmtree(out_dir, ignore_errors=True)
...
def test_release_merge_handoff_blocks_blocker_inventory_candidate_mismatch(
    ...
    try:
        ...
        inventory["tracked_candidates"]["latest_candidate"] = "v0.95.0-source"
        inventory["tracked_candidates"]["latest_stacked_pr"] = 116
