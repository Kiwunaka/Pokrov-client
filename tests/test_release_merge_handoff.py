...
def _merge_order_summary(ok: bool = True) -> dict:
    return {
        ...
        "stack_count": 11,
        "latest_pr": 71,
        "latest_candidate": "v0.51.0-source",
        "errors": [] if ok else ["PR #71 base must equal previous head"],
    }
...
def _github_status_summary(ok: bool = True) -> dict:
    return {
        ...
        "latest_pr": 71,
        "latest_candidate": "v0.51.0-source",
        "clean_pr_count": 11 if ok else 10,
        "draft_pr_count": 0,
        "unclean_pr_count": 0 if ok else 1,
        "successful_check_count": 33 if ok else 32,
        "failed_check_count": 0 if ok else 1,
        "errors": [] if ok else ["PR #71 check 'Flutter analyze and tests' is FAILURE"],
    }
...
def _stale_tag_readiness_summary() -> dict:
    ...
    summary["tag"] = "v0.50.0-source"
    summary["latest_candidate"] = "v0.50.0-source"
    return summary
...
def _write_input_summaries(
    ...
    *,
    ...
    tag_path = tmp_path / "v0.51.0-source-tag-readiness.json"
...
def test_release_merge_handoff_writes_handoff_summary(tmp_path: Path) -> None:
    ...
    assert summary["latest_candidate"] == "v0.51.0-source"
    assert summary["latest_pr"] == 71
...
def test_release_merge_handoff_uses_seed_default_input_paths() -> None:
    ...
    default_tag_path = (
        ...
        / "v0.51.0-source-tag-readiness.json"
    )
