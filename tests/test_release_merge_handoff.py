...
def _merge_order_summary(ok: bool = True) -> dict:
    return {
        ...
        "stack_count": 33,
        "latest_pr": 93,
        "latest_candidate": "v0.73.0-source",
        "errors": [] if ok else ["PR #93 base must equal previous head"],
    }
...
def _github_status_summary(ok: bool = True) -> dict:
    return {
        ...
        "latest_pr": 93,
        "latest_candidate": "v0.73.0-source",
        "clean_pr_count": 27 if ok else 26,
        "draft_pr_count": 0,
        "unclean_pr_count": 0 if ok else 1,
        "successful_check_count": 99 if ok else 98,
        "failed_check_count": 0 if ok else 1,
        "errors": [] if ok else ["PR #93 check 'Flutter analyze and tests' is FAILURE"],
    }
    ...
    *,
    ...
    if canonical_roots:
        ...
        tag_path = (
            ...
            / "test-inputs"
            / suffix
            / "v0.73.0-source-tag-readiness.json"
        )
        publication_path = (
            ROOT
            / "build"
            ...
            / "test-inputs"
            / suffix
            / "v0.73.0-source-publication-dry-run.json"
        )
    else:
        merge_path = tmp_path / "release-merge-order.json"
        github_path = tmp_path / "release-stack-github-status.json"
        tag_path = tmp_path / "v0.73.0-source-tag-readiness.json"
        publication_path = tmp_path / "v0.73.0-source-publication-dry-run.json"
...
def test_release_merge_handoff_writes_handoff_summary(tmp_path: Path) -> None:
    ...
    assert summary["latest_pr"] == 93
    assert summary["blocker_inventory_latest_candidate"] == "v0.73.0-source"
    assert summary["blocker_inventory_latest_pr"] == 93
...
def test_release_merge_handoff_blocks_input_summary_errors(
    ...
    merge_summary["errors"] = ["PR #93 base was stale when checked"]
    ...
    assert summary["input_errors"] == ["PR #93 base was stale when checked"]
...
def test_release_merge_handoff_blocks_blocker_inventory_candidate_mismatch(
    ...
    try:
        ...
        inventory["tracked_candidates"]["latest_candidate"] = "v0.74.0-source"
        inventory["tracked_candidates"]["latest_stacked_pr"] = 94
